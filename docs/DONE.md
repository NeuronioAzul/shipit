...
> Plano: [plan-shipit45 — divisões de dias na Linha do Tempo](plans/plan-shipit45-dashboardTimelineDayColumns.prompt.md)

- [x] **feat(dashboard):** Grade de dias na Linha do Tempo — gráfico extraído de `DashboardPage.tsx` para `src/components/TimelineChart.tsx` (props `activities`/`monthRef`/`onSelect`), com layout em duas colunas (gutter `w-35` + trilha `flex-1 min-w-0 relative @container`) para cabeçalho, colunas de fundo e barras compartilharem a mesma escala `100/n %` (corrige o desalinhamento anterior, em que o cabeçalho era `%` da linha inteira incluindo o gutter de 140px). Camada `#dashboard-gantt-columns` (`absolute inset-0 pointer-events-none`) com uma coluna por dia: fim de semana `bg-muted/40`, hover `bg-primary/10`, divisória `border-l border-border/60` só no modo completo; cabeçalho `#dashboard-gantt-header` com número (`text-[10px]`) + letra pt-BR (`text-[9px]`), fim de semana `text-accent font-medium`, hover `text-primary font-semibold`, `title` com a data longa. Densidade por **container query do Tailwind v4** (`@min-[682px]:` = 22px × 31 dias): abaixo disso, sem linhas/letras e números só nos marcos (1, 5, 10…). Hover calculado em `onMouseMove` na trilha (`getDayAtPointer` sobre `getBoundingClientRect`), estado isolado no componente. Novo `src/utils/timelineDays.ts` (`WEEKDAY_LETTERS`, `MIN_DAY_WIDTH_PX`, `FULL_DENSITY_MIN_TRACK_PX`, `getTimelineDays`, `getDayAtPointer`, `getActivityDays` movido do Dashboard, `formatShortDate`); `parseMonthReference` exportado de `monthReference.ts`. Tooltip da barra passa a usar `htmlToPlainText`. Testes: 12 em `timelineDays.test.ts`, 6 em `TimelineChart.test.tsx` (jsdom), 1 cenário E2E em `app.spec.ts` (contagem de dias/colunas, letra do dia 1, fins de semana, hover, alinhamento barra × colunas com ±1px, modo compacto a 800×600). Conferência visual nos 10 temas via screenshots.
- [x] **fix(release):** Vazamento de preâmbulo da IA no CHANGELOG ("Segue a entrada do CHANGELOG da versão 1.16.0...") — em `docs/scripts/release_v2.py`, a regra de formato da resposta foi movida para o FIM do prompt de `_generate_changelog_with_claude` (começar direto pela frase de abertura ou pelo primeiro `### `, sem apresentação/fechamento/menção às fontes) e a resposta passa por `_clean_ai_changelog_entry` (remove `## [x.y.z]` indevido, parágrafos de apresentação antes do primeiro heading via `AI_CHANGELOG_PREAMBLE_RE` e fechamentos após o último bullet via `AI_CHANGELOG_TRAILER_RE`, preservando a frase de abertura legítima). `_normalize_ai_commit_message` descarta linhas anteriores ao primeiro cabeçalho Conventional Commits válido. Linha vazada removida da entrada 1.16.0 do CHANGELOG.
...
- [x] **feat:** Guarda declarativa de schema (`UNSUPPORTED_LEGACY_SCHEMA` + `findUnsupportedLegacySchema` em `database.ts`): banco anterior à 1.14.x → `dialog.showErrorBox` com instrução de instalar a 1.14.x + `app.quit()` antes do `synchronize()`, sem tocar no arquivo (suprimido sob `PLAYWRIGHT=1`). Testes em `database.test.ts` + novo `e2e/legacy-schema-guard.spec.ts` (semeia banco legado via `e2e/fixtures/legacyDatabase.ts`, confere encerramento e hash SHA-256 intacto).
- [x] **chore(release):** Geração de textos do `docs/scripts/release_v2.py` reescrita. Step 2-3 (commit): a IA recebe o **diff real** das mudanças staged (`_collect_staged_diff_for_ai`, com orçamento por arquivo/total e exclusão de gerados/binários) + commits recentes como referência, e devolve a mensagem completa Conventional Commits (cabeçalho `tipo(escopo): assunto`, corpo em bullets com o porquê, rodapé `BREAKING CHANGE`); cabeçalho validado por `_conventional_commit_problems` (tipo, escopo, ≤72 chars, minúscula, sem ponto, linha em branco antes do corpo), revisão `s/e/r/m/a`, edição no editor externo (`_edit_text_in_editor`, helper compartilhado com o CHANGELOG) e commit via `git commit -F`. Step 5 (CHANGELOG): fontes passam a ser a seção `[Unreleased]` (principal) + commits **após a data da última tag** com assunto e corpo (`_collect_release_context`; `tag..HEAD` trazia o histórico inteiro do `dev` por causa do squash merge), sem commits de mecânica de release; prompt orientado ao usuário final (seções Keep a Changelog + `### Atenção` opcional, bullets `**Título.** explicação`, proibição de detalhes de desenvolvimento); ao inserir a versão, `[Unreleased]` é esvaziada (com confirmação). Release notes (Step 13) sem parágrafo genérico. `_run_claude`: prompts grandes vão por STDIN (limite da linha de comando), novo `SHIPIT_CLAUDE_MODEL` (`--model`) e erro do CLI lido também do STDOUT.

---

## v1.14.0 — 2026-09-11

> Plano: [plan-shipit42 — publicações por ambiente](plans/plan-shipit42-deploymentsPerEnvironment.prompt.md)

- [x] **feat:** Publicações por ambiente em uma única atividade — coluna JSON `deployments` (`{ ambiente: releases[] }`) na `Activity` substitui `environment` + `svn_releases`; utilitário `src/utils/deployments.ts` (parse/serialize/ordem fixa dsv→hmg→prd/migração legada); `DeploymentsEditor` (toggle colorido + `InputTags` por ambiente, releases retidas ao desmarcar, atalho "Repetir releases") na seção única "Publicações por ambiente (uso interno)"; `DeploymentPipeline` (dsv › hmg › prd, releases copiáveis, slots não publicados apagados) na lista e no detalhe; filtro "Ambiente" na lista; busca global/local por `deployments`. Removidos `EnvironmentSelector`/`EnvironmentBadge`. Fora do DOCX (teste de regressão atualizado).
- [x] **feat:** Aviso bloqueante + backup + migração na primeira abertura — `initDatabase` dividido em `openDatabase` (sem sync) / `needsLegacyMigration` (PRAGMA) / `migrateLegacyEnvironmentColumns` (SQL cru, idempotente) / `finalizeDatabase` (`synchronize()`), com guarda em `getDb()` enquanto pendente; `electron/db-backup.ts` (cópia verificada de `shipit.db` + sidecars, sem resíduo em falha); `MigrationGate` montado por `App.tsx` no lugar de layout/rotas (contagem de 30 s, etapas, caminho do backup, instruções de downgrade, erro sem alteração quando o backup falha); handlers `app:getStartupMigration`/`app:runStartupMigration`/`app:getLastMigrationNotice`/`app:openReleasesPage`; chaves internas `__migrationNotice`/`__lastRunVersion` em `settings.json` (preservadas por `saveSettings`); seção "Backup do banco de dados" em Configurações. Releases sem ambiente descartadas (decisão do usuário). Testes unitários (`deployments`, `db-backup`, `database` migração, `MigrationGate`, `DeploymentsEditor`, `DeploymentPipeline`) + E2E (`app.spec.ts` atualizado, novo `migration.spec.ts` com banco legado semeado via `sql.js`).
