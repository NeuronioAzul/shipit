...
| `Activity` | `activities` | UUID v7 | Atividade registrada com período, status e `deployments` (JSON interno `{ ambiente: releases[] }` — publicações por ambiente com releases SVN; não exportado no DOCX) |
